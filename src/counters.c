...
static void *StatsMgmtThread(void *arg)
{
    ...
    // TODO - Question may I update this error message? There doesn't seem to be a
    // StatsInitCounterApi any longer, but there **is** one StatsInit()
    if (stats_ctx == NULL) {
...
static int StatsTableRecordsSortByName(const void *a, const void *b)
{
    ...
    if (s0->name != NULL && s1->name != NULL) {
        if (strcmp(s0->tm_name, s1->tm_name) == 0) {
...
static int StatsOutput(ThreadVars *tv)
{
    ...
    uint64_t table_size = stats_table.ntstats * stats_table.nstats;

    qsort(stats_table.tstats, table_size, sizeof(StatsRecord), StatsTableRecordsSortByName);
