...
uint16_t GetTargetFlagFromTargetEnum(enum ExceptionPolicyTargetValues target_flag)
{
    switch (target_flag) {
        case EXCEPTION_TARGET_ENUM_NONE:
            return 0;
        case EXCEPTION_TARGET_ENUM_DEFRAG_MEMCAP:
            return EXCEPTION_TARGET_FLAG_DEFRAG_MEMCAP;
        case EXCEPTION_TARGET_ENUM_SESSION_MEMCAP:
            return EXCEPTION_TARGET_FLAG_SESSION_MEMCAP;
        case EXCEPTION_TARGET_ENUM_REASSEMBLY_MEMCAP:
            return EXCEPTION_TARGET_FLAG_REASSEMBLY_MEMCAP;
        case EXCEPTION_TARGET_ENUM_FLOW_MEMCAP:
            return EXCEPTION_TARGET_FLAG_FLOW_MEMCAP;
        case EXCEPTION_TARGET_ENUM_MIDSTREAM:
            return EXCEPTION_TARGET_FLAG_MIDSTREAM;
        case EXCEPTION_TARGET_ENUM_APPLAYER_ERROR:
...
static uint16_t ExceptionPolicyFlag(enum PacketDropReason drop_reason)
{
    switch (drop_reason) {
        ...
        case PKT_DROP_REASON_APPLAYER_ERROR:
            return EXCEPTION_TARGET_FLAG_APPLAYER_ERROR;
        default:
            return (uint16_t)EXCEPTION_TARGET_ENUM_NONE;
    }
